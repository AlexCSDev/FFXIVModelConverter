...
namespace FFXIVModelConverter
{
    internal class ModelLoader
    {
        ...
        public async Task<TTModel> LoadModel(string mdlPath, string inGamePath, Action<bool, string> loggingFunction = null)
        {
            var mdl = await LoadRawModelData(mdlPath, inGamePath);
            var ttModel = TTModel.FromRaw(mdl, loggingFunction);
            return ttModel;
        }
        ...
        private async Task<XivMdl> LoadRawModelData(string mdlPath, string inGamePath)
        {
            //TODO: validate inGamePath
            var xivMdl = new XivMdl { MdlPath = inGamePath };
            int totalNonNullMaterials = 0;

            var meshCount = 0;
            using (var r = new BinaryReader(File.OpenRead(mdlPath)))
            {
                var header = MdlStructs.ModelFileHeader.Read(r);

                for (var i = 0; i < header.VertexDeclarationCount; ++i)
                {
                   _ = MdlStructs.VertexDeclarationStruct.Read(r);
                }
            ...
            using (var br = new BinaryReader(File.OpenRead(mdlPath)))
            {
                ...
                short vertexDeclarationCount = br.ReadInt16();
                br.BaseStream.Seek(54, SeekOrigin.Current); //skip everything else in the header
                br.BaseStream.Seek(17 * 8 * vertexDeclarationCount, SeekOrigin.Current); //skip vertex declarations*/
